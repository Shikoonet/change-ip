...
**نیمه‌ی DNS هنوز تست‌نشده است** — آن اجرا `provider_only` بود و Cloudflare اصلاً صدا زده
نشد. `cloudflare_replace_ip_step.yml` هنوز صفر فراخوانی زنده دارد. به معنای این ریپو یعنی
